...
namespace opentissue {
    namespace math {
        ...
        class Vector {
        protected:
            typedef typename opentissue::math::ValueTraits<value_type_> value_traits_;
        ...
        template <typename T> inline Vector<T> sgn(Vector<T> const &v) {
            using opentissue::math::sgn;
            return Vector<T>(sgn(v(0)), sgn(v(1)), sgn(v(2)));
        }
